...
$STYLES = array(
	...
	'font-awesome' => array(
		'src' 	=> PT_URL . '/third_party/font-awesome/css/font-awesome.min.css',
		'deps' 	=> false,
		'ver'	=> '4.1.0',
		'media' => false		
	),
...
$SCRIPTS = array(
	'bootstrap' => array(
		'src' 	=> PT_URL . '/third_party/bootstrap/js/bootstrap.min.js',
		'deps' 	=> false,
		'ver'	=> '3.2.0',
		'in_footer' => true
	),
...
add_action( 'admin_enqueue_scripts', 'pt_load_third_party' );


function wlt_builder_scripts() {
	
	// ONLY SHOW IF ITS A PAGE
	if( is_page()  ){
		wp_enqueue_style( 'wlt_builder', PT_URL . '/assets/css/frontend/wlt_builder.css' );
	}

}

add_action( 'wp_enqueue_scripts', 'wlt_builder_scripts' );
 
function pt_load_third_party(){	
	global $STYLES;
	global $SCRIPTS, $pagenow;
 
	
	if( is_page() || ( is_admin() && ( $pagenow == "post.php" || $pagenow == "post-new.php" ) ) ){
	
		foreach( $STYLES as $handle => $style_data ){
			wp_register_style( 'pt-'.$handle.'-css', $style_data['src'], $style_data['deps'], $style_data['ver'], $style_data['media'] );
		}
	
		foreach( $SCRIPTS as $handle => $script_data ){
			wp_register_script( 'pt-'.$handle.'-js', $script_data['src'], $script_data['deps'], $script_data['ver'], $script_data['in_footer'] );
		}
...
/* HELPER FUNCTION */
function pt_calculate_column_width( $width ){
	$temp = explode( "/", $width );

	if( sizeof( $temp ) == 2 ){
		return 12*$temp[0]/$temp[1];
	}
	else{
		return 12;
	}
...
function pt_random_string( $length = 10 ){
    $characters = '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ';
    $random_string = '';
    for ($i = 0; $i < $length; $i++) {
        $random_string .= $characters[rand(0, strlen($characters) - 1)];
    }
    return 'pt'.$random_string;
}

function pt_load_classes( $folder, $instantiate = false ){
	foreach ( glob( $folder."*.php" ) as $filename ){
		include $filename;
		if( $instantiate == true ){
			$filename = explode( '/', $filename );
			$class_name = str_replace( '.php', '', $filename[ sizeof($filename) - 1 ] );
			new $class_name;
		}
	}
}

function pt_load_dependencies( $dependencies ){
	/* enqueue styles */
	if( !empty( $dependencies['styles'] ) ){
		foreach( $dependencies['styles'] as $handle => $style_data ){							
			if( !empty( $style_data ) ){				
				wp_enqueue_style( $handle, $style_data['src'], $style_data['deps'], $style_data['ver'], $style_data['media'] );
			}
			else{
	...
	if( !empty( $dependencies['scripts'] ) ){
		foreach( $dependencies['scripts'] as $handle => $script_data ){
			if( !empty( $script_data ) ){
				wp_enqueue_script( $handle, $script_data['src'], $script_data['deps'], $script_data['ver'], $script_data['in_footer'] );
			}
			else{
