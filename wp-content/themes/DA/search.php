...
<?php if(!defined('HIDE_SEARCHRESULTS_BOX') && $wp_query->found_posts != 0 ){ ?>
    ...
    <h4><?php echo hook_gallerypage_results_text(str_replace("%a",number_format($wp_query->found_posts),$CORE->_e(array('gallerypage','1'))));
    // print_r($wp_query->found_posts);
    if ( $wp_query->found_posts < 5) {
        ?>
        <br/>
        <span class="cpm-search-msg">
            Contact us - you may qualify for 50% off a 1 year membership for being one of the first in your area!
        </span>
        <?php
    }
    ?></h4>
