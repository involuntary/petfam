...
class core_search {
    ...
    public static function instance() {
        if ( !self::$_instance ) {
            self::$_instance = new core_search();
        }
		...
		$flagHeading = false; $headingCount  = 0; $fieldsCounter = 0;
			...
			<div id="head_group_'.$headingCount.'" class="panel-collapse '.$status.'"> <div class="panel-body"> ';
  			...
  			continue; // continue to stop showing the label as well
				...
				printf( '<select name="%ss" class="form-control" id="core_country_dropdown'.$extraID.'"><option value="">&nbsp;&nbsp;</option> ', $field_name );
				...
				// echo '<script> jQuery(document).ready(function() { jQuery("#core_country_dropdown'.$extraID.'").select2({ tags: true }); });</script>';	 
                ...
                $selected = isset( $_GET[$field_name] ) ? $_GET[$field_name] : '';				 
                    ...
                    foreach($county_list AS  $cc){ 
                     $neworder[] = array("name" => $GLOBALS['core_country_list'][$cc], "key" => $cc );
                    }
                    
                    sort($neworder);
                    
					foreach($neworder AS $cc){ 
						...
						//if($field->key == "map-country1" && strlen($cc['key']) > 2){ continue; }
                        
						if(in_array($cc['key'],$gh)){ continue; }
						$gh[] = $cc['key'];
						
							if($selected == $cc['key']){ $sel = " selected=selected"; }else{ $sel = ""; }
						
							if(isset($GLOBALS['core_country_list'][$cc['key']])){
								$CSTR .= "<option value='".$cc['key']."' ".$sel.">".$GLOBALS['core_country_list'][$cc['key']]."</option>";
							}else{
								$CSTR .= "<option value='".$cc['key']."'".$sel.">".$cc['name']."</option>";
							}
					...
					printf( '<select name="%s" class="form-control" id="core_'.$field->key.'_dropdown'.$extraID.'" onChange="Search_ChangeState(\''.$extraID.'\',\''.str_replace("http://","",get_home_url()).'\')"><option value="">&nbsp;&nbsp;</option> ', $field_name );
					echo $CSTR;
					echo '</select>';
    ...
    return date.valueOf() <= checkin.date.valueOf() ? \'disabled\' : \'\';
                    ...
                    case 'taxonomy':
                        ...
                        if ( isset($taxo->hierarchical) && $taxo->hierarchical ) {
							...
							$args['show_count'] = hook_displaycount(1);
                            ...
                            if ( $field->field_type == 'multi' ) {
								 ...
								 // FIND SELECTED ITEMS
								if (isset($_GET[$field_name]) && is_array($_GET[$field_name]) && !empty($_GET[$field_name])  ) {
								 
									foreach ($_GET[$field_name] as $key => $post_term) { 
								...
								$dropdown = str_replace(sprintf("<select  name='%s'", $field_name), sprintf("<select name='%s[]' multiple='multiple' class='select2-".$field_name." form-control' ", $field_name), $dropdown);
								
								$dropdown = str_replace("<select","<select style='width:100%'", $dropdown);
								
								$dropdown .= '<script> jQuery(document).ready(function() { jQuery(".select2-'.$field_name.'").select2({ tags: true }); });</script>';
...
<label class="col-md-12"><?php echo $field->label; ?></label>
...
<div id="head_group_cat"> 
							 ...
							 if ( $count > 0 ){
								 ...
								 foreach ( $terms as $term ) {								 
								   ...
								   echo '<div class="row">
										...
										echo ' <span class="pull-right">('.$term->count.')</span> ';
		...
		if(isset($GLOBALS['CORE_THEME']['layout_columns']['3columns']) && $GLOBALS['CORE_THEME']['layout_columns']['3columns'] == "1"){ $ss = "col-md-9 col-sm-9"; }else{ $ss = "col-md-9 col-sm-9"; }
		...
		// STAR RATING SYSTEM
			...
			echo '<div class="form-group starratingpanel ">
			
			 
				 
					<label> '.$CORE->_e(array('widgets','28')).' </label>
			...
			echo '<div class="row">
				...
				if(isset($GLOBALS['CORE_THEME']['layout_columns']['3columns']) && $GLOBALS['CORE_THEME']['layout_columns']['3columns'] != "1"){ 
					echo '<div class="col-md-4 col-sm-12  col-xs-12"><small>'.$i.' '.$CORE->_e(array('widgets','29')).'</small></div>';
				}
...
$advance_search = core_search::instance();
