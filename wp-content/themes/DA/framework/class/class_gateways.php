...
function gateway_paypal($data=""){
	...
	$GLOBALS['total'] = number_format(str_replace(",","",$GLOBALS['total']),2);
...
function core_generic_gateway_callback($orderid, $orderdata ){ global $wpdb, $CORE, $userdata; $order_data_description = "";
	...
	if(substr($obits[0],0,4) == "CART"){ 
			...
			if(defined('WLT_CART') && strlen($obits[1]) > 4 ){		
				...
				if(!empty($hassession)){
					...
					// NOW WE LOOP ALL ITEMS AND REMOVE THE QTY IF REQUIRED
						if(isset($cart_data['items']) && is_array($cart_data['items'])){
							...
							// CHECK FOR COMMENTS
							if($saveme_orderdata['username'] == "Guest"){		
							...
							$order_data_description .= "Shipping : ".hook_price($orderdata['shipping'])."\n";
							...
							foreach($cart_data['items'] as $key=>$item){
								foreach($item as $mainitem){
									...
									// ADD ON PRICE FOR CUSTOM ATTRIBUTES
									$extraprice=0;
									$has_attributes = get_post_meta($key,"wlt_productattributes",true);										 						
									if(is_array($has_attributes)){
									
									// CLEAN UP CUSTOM TEXT
									$hh = explode(":",$mainitem['custom']);	
									$issetg = false;
									 	 
											foreach($has_attributes['name'] as $k=>$att ){		
										 														
												if(trim($att) == trim($hh[0]) ){
																							 
													$options = explode( PHP_EOL, $has_attributes['value'][$k] );
													...
													if(is_array($options) && !empty($options)){
														...
														preg_match_all("^\[(.*?)\]^",$oval,$fields, PREG_PATTERN_ORDER);
														   
														  	$match1 = trim(preg_replace('/[\[{\(].*[\]}\)]/U', '', $oval ));
															$match2 = trim(preg_replace('/[^A-Za-z-0-9 \-]/', '', trim($hh[1])));
															
															//echo $match1." / ".$match2."<br>"; //
														  	
															if(isset($fields[1][0]) && strpos($match2, $match1) !== false && !$issetg ){
															 
																$extraprice += $fields[1][0]; 
																$issetg = true; 
															}
									...
									$order_data_description .= "Unit Price: ".hook_price(get_post_meta($key,'price',true)+($extraprice))."\n";
									...
									$order_data_description .= "Total: ".hook_price((get_post_meta($key,'price',true)+$extraprice)*$mainitem['qty'])."\n\n";	
			 ...
			 //die($order_data_description." == ".$extraprice." == /".get_post_meta($key,'price',true));
	...
	// CHECK IF THIS IS A MEMBERSHIP PAYMENT
			...
			// GET CURRENT MEMBERSHIP TIME
			$current_time = get_user_meta($obits[2],'wlt_membership_expires',true);
			if($current_time == "" ||  strtotime($current_time) < strtotime(current_time( 'mysql' )) ){ $current_time = current_time( 'mysql' ); }
			 
			// UPDATE TIME
			if($membershipfields[$obits[1]]['expires'] == ""){ $expire_days = 30;  }else{ $expire_days = $membershipfields[$obits[1]]['expires']; }			
			update_user_meta( $obits[2], 'wlt_membership_expires', date("Y-m-d H:i:s", strtotime($current_time . " +".$expire_days." days")) );	
			
			// ADD ON START DATE
			update_user_meta( $obits[2], 'wlt_membership_started', date("Y-m-d H:i:s") );
			
			// UPDATE ALL MEMBERS LISTINGS SO THEIR LISTINGS DONT EXPIRE
			$SQL = "UPDATE ".$wpdb->prefix."postmeta 
			INNER JOIN ".$wpdb->prefix."posts ON (".$wpdb->prefix."posts.ID = ".$wpdb->prefix."postmeta.post_id AND ".$wpdb->prefix."posts.post_author = ".$obits[2]." ) 
			SET ".$wpdb->prefix."postmeta.meta_value = '".date("Y-m-d H:i:s", strtotime(current_time( 'mysql' ) . " +".$expire_days." days"))."' 
			WHERE ".$wpdb->prefix."postmeta.meta_key = 'expiry_date' AND ( ".$wpdb->prefix."postmeta.meta_value < '".current_time( 'mysql' )."' OR ".$wpdb->prefix."postmeta.meta_value = '' )";
	...
	// EXTRA FOR CART
	if(isset($guestcheckoutemail)){
	$orderdata['email'] = $guestcheckoutemail;
	}
	...
	// REPLACE NEW ORDER ID
	$order_data_description = str_replace("XXORDERIDXX","0000".$GLOBALS['new_orderid'],$order_data_description);
	...
	$_POST['orderid'] 			= "0000".$GLOBALS['new_orderid']; //$orderid;
	$_POST['paid_item_id'] 		= $obits[1];
	$_POST['order_email'] 		= $orderdata['email'];
