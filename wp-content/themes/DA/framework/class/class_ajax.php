			...
			switch($_GET['action']){
				...
				case "SaveRating": {
					...
					if(!is_array($rated_user_ips)){ $rated_user_ips = array(); }
						...
						if(isset($rated_user_ips[$_GET['pid']]) && isset($rated_user_ips[$_GET['pid']]['ips']) && in_array($user_ip, $rated_user_ips[$_GET['pid']]['ips']) ){							
							echo '<i class="fa fa-times"></i> '.$CORE->_e(array('widgets','37'));
							die();
					...
					echo '<i class="fa fa-check-square-o"></i> '.$CORE->_e(array('widgets','36'));
					...
					if(is_numeric($_GET['pid']) ){
						...
						if(isset($rated_user_ips[$_GET['pid']]) && in_array($user_ip, $rated_user_ips[$_GET['pid']]['ips']) ){ 
							...
							if($result == ""){ echo 0; }else{ 
								// REUTNR RATING BAR
								if(!defined('WLT_DATING') && defined('WLT_VIDEO') && ( $GLOBALS['CORE_THEME']['rating_type'] == 8 || $GLOBALS['CORE_THEME']['rating_type'] == 9 ) ){
									echo '<div class="progress">'.$CORE->_e(array('widgets','37')).'</div>';
								}else{
									 echo $result;
								}
							}
				...
				if($_GET['id'] == "cartcomment"){
					...
					if($userdata->ID){
						...
						$SQL = "SELECT * FROM ".$wpdb->prefix."core_sessions WHERE session_key = ('".strip_tags($_GET['ses'])."') LIMIT 1";
						$hassession = $wpdb->get_results($SQL, OBJECT);
						if(!empty($hassession)){
							$cart_data 				= unserialize($hassession[0]->session_data);
							$cart_data['comments'] 	= stripslashes(strip_tags($_GET['value']));
							
							$wpdb->query("UPDATE ".$wpdb->prefix."core_sessions SET session_data = '".serialize($cart_data)."' WHERE session_key = ('".strip_tags($_GET['ses'])."') LIMIT 1"); 
						...
						update_option('cartc_' . stripslashes(strip_tags($_GET['value'])) , stripslashes(strip_tags($_GET['ses'])) );
					 
					//die('cartc_' . stripslashes(strip_tags($_GET['em'])));
					} 
					
				}elseif($userdata->ID){		
					update_user_meta($userdata->ID, strip_tags($_GET['value']), strip_tags($_GET['ses']));
					//echo $userdata->ID."<--";
				}				
				} break;			
				case "CatUpdatePrice": {
