...
if(isset($_POST['coupon_import']) && strlen($_POST['coupon_import']) > 2 ){
	...
	// SAVE ORDER SETTINGS
		...
		$SQL = "UPDATE ".$wpdb->prefix."core_orders SET 
			...
			order_data='".esc_sql($_POST['order_data'])."',		
		  	order_status='".esc_sql($_POST['order_status'])."',
			order_tax='".esc_sql($_POST['order_tax'])."',
			order_shipping='".esc_sql($_POST['order_shipping'])."',
			order_total='".esc_sql($_POST['order_total'])."',
			order_email = '".esc_sql($_POST['order_email'])."',			
			shipping_label='".esc_sql($_POST['shipping_label'])."',
			
			payment_data='".esc_sql($_POST['payment_data'])."'
			WHERE autoid='".$_POST['autoid']."' LIMIT 1";
...
<a data-toggle="modal" href="#VideoModelBox" class="btn btn-warning youtube" onclick="PlayPPTVideo('0WHCeJ82jY4','videoboxplayer','479','350');" style="float:right; ">Watch Video Tutorial</a>
